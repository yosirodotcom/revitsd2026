...
* **Gejala**: Ketika Super Admin menambahkan foto dokumentasi rapat, gambar tersebut tampil normal di browser miliknya sendiri. Namun, saat user lain (misal: Fasilitator / Rizal) membuka agenda rapat tersebut, gambar dokumentasi rapat pecah/tidak tampil (menampilkan ikon gambar rusak dan teks alternatif *"Dokumentasi Rapat"*).
* **Penyebab**: 
  ...
  3. **Tautan Langsung Google Drive Terblokir**: Setelah diunggah ke Drive, Apps Script mengembalikan URL unduhan langsung (`file.getDownloadUrl()`). Browser memblokir tautan ini di dalam tag `<img>` karena dianggap sebagai lampiran (*attachment download*) dengan masalah CORS/pengalihan.
* **Solusi**:
